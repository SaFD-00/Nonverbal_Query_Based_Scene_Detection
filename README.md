...
## 4. Repository Structure (예시)

> 실제 디렉토리 구조는 코드 기준으로 수정해 사용하세요.

```text
Nonverbal_Query_Based_Scene_Detection/
├── ShotDetector/
│   └── Debug/
│       └── video/                # Shot / Scene Detection용 raw videos
├── mmv/                          # MMV 모듈 및 전처리/학습 코드
├── data/
│   ├── qvhighlights/             # QVHighlights dataset
│   └── ...                       # OVSD, YouTube 등 기타 데이터
├── img/                          # README에 사용되는 파이프라인/데이터셋 이미지
└── README.md
